from naive_factorization import NaiveNet, create_csv_logger_cb
...
def train(start_rank, tolerance, load_model, dim_layer):
    ...
    filename = "naive_sr" + str(start_rank) + "_v" + str(tolerance)
    folder_name = "naive_sr" + str(start_rank) + "_v" + str(tolerance) + '/latest_model'
    folder_name_best = "naive_sr" + str(start_rank) + "_v" + str(tolerance) + '/best_model'

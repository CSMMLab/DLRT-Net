...
class PartialDLRANet(nn.Module):
    """
    The network is build of a set of normal Resnet layers f(Ax+b)+x and a prescribed number of
    Resnet Low-Rank layers.
    The layout is given by Input->ResNet->DLRA->...->DLRA->ResNet->...->ResNet->Ouput

    Weight updates happen in a splitting manner:
    """
    weights_K: list  # [torch.Tensor]  # n1 x r
    weights_Lt: list  # [torch.Tensor]  # n2 x r
    weights_S: list  # [torch.Tensor]  # r x r
    ...
    biases: list  # [torch.Tensor]  # n2
    num_layers_dlra: int
    layer_width_dlra: int

    weigths_W: list  # n1 x n2
    biases_W: list  # n2
    num_layers_normal: int
    layer_width_normal: int
    ...
    optim_Lt: torch.optim.SGD  # optimizer for L step
    optim_S: torch.optim.SGD  # optimizer for S step
    optim_b: torch.optim.SGD  # optimizer for biases
    optim_W: torch.optim.SGD  # optimizer for W
    optim_Wb: torch.optim.SGD  # optimizer for biases

    def __init__(self, input_dim: int, output_dim: int, layer_width_dlra: int, num_layers_dlra: int,
                 low_rank: int, layer_width_normal: int, num_layers_normal: int):
        self.num_layers_dlra = num_layers_dlra
        self.layer_width_dlra = layer_width_dlra
        self.num_layers_normal = num_layers_normal
        self.layer_width_normal = layer_width_normal
        self.num_layers_total = self.num_layers_dlra + self.num_layers_normal

        # weight initialization

        # 1) Weight initialization for first dense ResNet layer
        self.weights_W = [torch.rand(input_dim, layer_width_dlra)]
        self.biases_W = [torch.rand(layer_width_dlra)]

        # 2) Weight initialization for first DLRA ResNet layer
        W = torch.rand(layer_width_dlra, layer_width_dlra)
        W = W / torch.norm(W)
        u, s, v = torch.svd(W)
        vt = torch.transpose(v, 0, 1)
        self.weights_K = [torch.rand(layer_width_dlra, low_rank)]  # gets overwritten in K-step
        self.weights_Lt = [torch.rand(layer_width_dlra, low_rank)]  # gets overwritten in L-step
        self.weights_S = [
            torch.narrow(torch.narrow(torch.diag(s), 0, 0, low_rank), 1, 0, low_rank)]  # narrow to rxr
        self.aux_U = [torch.narrow(u, 1, 0, low_rank)]  # narrow to n2xr
        self.aux_Unp1 = [torch.rand(layer_width_dlra, low_rank)]  # gets overwritten
        self.aux_Vt = [torch.narrow(vt, 0, 0, low_rank)]  # narrow to rxn1
        self.aux_Vtnp1 = [torch.rand(low_rank, layer_width_dlra)]  # gets overwritten
        self.aux_N = [torch.rand(low_rank, low_rank)]  # gets overwritten
        self.aux_M = [torch.rand(low_rank, low_rank)]  # gets overwritten
        self.biases = [torch.rand(layer_width_dlra)]

        # 3) Weight initialization for other DLRA ResNetLayers
        for i in range(1, num_layers_dlra):
            # weight initialization
            self.W = torch.rand(layer_width_dlra, layer_width_dlra)
            self.W = self.W / torch.norm(self.W)
            u, s, v = torch.svd(self.W)
            vt = torch.transpose(v, 0, 1)
            self.weights_K.append(torch.rand(layer_width_dlra, low_rank, requires_grad=True))
            self.weights_Lt.append(torch.rand(layer_width_dlra, low_rank, requires_grad=True))
            self.weights_S.append(torch.narrow(torch.narrow(torch.diag(s), 0, 0, low_rank), 1, 0, low_rank))
            self.aux_U.append(torch.narrow(u, 1, 0, low_rank))
            self.aux_Unp1.append(torch.rand(layer_width_dlra, low_rank, requires_grad=False))
            self.aux_Vt.append(torch.narrow(vt, 0, 0, low_rank))
            self.aux_Vtnp1.append(torch.rand(low_rank, layer_width_dlra, requires_grad=False))
            self.aux_N.append(torch.rand(low_rank, low_rank, requires_grad=False))
            self.aux_M.append(torch.rand(low_rank, low_rank, requires_grad=False))
            self.biases.append(torch.rand(layer_width_dlra, requires_grad=True))

        # 4) Weight initialization for other ResNet Layers
        for i in range(1, num_layers_normal - 1):
            self.weights_W.append(torch.rand(layer_width_dlra, layer_width_normal))
            self.biases_W.append(torch.rand(layer_width_normal))
        self.weights_W.append(torch.rand(layer_width_normal, output_dim))
        self.biases_W.append(torch.rand(output_dim))

        with torch.no_grad():
            for i in range(0, self.num_layers_dlra):
                # mark for auto differentiation tape
                self.weights_K[i].requires_grad = True
                self.weights_Lt[i].requires_grad = True
                self.weights_S[i].requires_grad = True
                ...
                self.aux_M[i].requires_grad = False
                self.biases[i].requires_grad = True
            for i in range(0, self.num_layers_normal):
                self.weights_W[i].requires_grad = True
                self.biases_W[i].requires_grad = True

        # Create optimizers
        self.optim_K = torch.optim.SGD(self.weights_K, lr=1e-3)
        self.optim_Lt = torch.optim.SGD(self.weights_Lt, lr=1e-3)
        self.optim_S = torch.optim.SGD(self.weights_S, lr=1e-3)
        self.optim_W = torch.optim.SGD(self.weights_W, lr=1e-3)

        self.optim_b = torch.optim.SGD(self.biases, lr=1e-3)
        self.optim_Wb = torch.optim.SGD(self.biases_W, lr=1e-3)

    def w_and_b_update(self):
        """
        @brief: updates all non-dlra weights and biases
        :return: None
        """
        self.optim_b.step()
        self.optim_W.step()
    ...
    def k_step_forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            for i in range(0, self.num_layers_dlra):
                ...
                self.weights_K[i].requires_grad = True
        z = input_tensor
        # 2) pass forward z = f(xW+b)
        z = F.relu(torch.matmul(z, self.weights_W[0]) + self.biases_W[0])
        for i in range(0, self.num_layers_dlra):
            # pass forward z = f(xW+b) +x = f(xKV + b) +x
            z = F.relu(torch.matmul(z, torch.matmul(self.weights_K[i], self.aux_Vt[i]))) + z
        # intermediate layer
        z = F.relu(torch.matmul(z, self.weights_W[1]) + self.biases_W[1])
        for i in range(2, self.num_layers_normal - 1):
            z = F.relu(torch.matmul(z, self.weights_W[i]) + self.biases_W[i]) + z
        # output layer
        z = torch.matmul(z, self.weights_W[self.num_layers_normal - 1]) + self.biases_W[self.num_layers_normal - 1]
        return z  # F.log_softmax(z, -1)
    ...
    def s_step_forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(0, self.num_layers_dlra):
            # z = f(xW+b)+x \approx f(xUnp1 S Vnp1^T + b) +x
            z = F.relu(
                torch.matmul(z, torch.matmul(self.aux_Unp1[i], torch.matmul(self.weights_S[i], self.aux_Vtnp1[i]))))
        # intermediate layer
        z = F.relu(torch.matmul(z, self.weights_W[1]) + self.biases_W[1])
...
class ReferenceDLRANet(nn.Module):
    """
    The network is build of a set of normal Resnet layers f(Ax+b)+x and a prescribed number of
    Resnet Low-Rank layers.
    The layout is given by Input->ResNet->DLRA->...->DLRA->ResNet->...->ResNet->Ouput
    ...
    def __init__(self, input_dim: int, output_dim: int, layer_width_dlra: int, num_layers_dlra: int,
        ...
        for i in range(1, num_layers_normal - 1):
            self.weights_W.append(torch.rand(layer_width_dlra, layer_width_normal))
            self.biases_W.append(torch.rand(layer_width_normal))
        self.weights_W.append(torch.zeros(layer_width_normal, output_dim))
        self.biases_W.append(torch.zeros(output_dim))
        ...
        self.optim_b = torch.optim.SGD(self.biases, lr=1e-5)
        self.optim_Wb = torch.optim.SGD(self.biases_W, lr=1e-5)

    def plain_forwad(self, z: torch.Tensor):
        for i in range(0, self.num_layers_normal - 1):
            z = F.relu(torch.matmul(z, self.weights_W[i]) + self.biases_W[i])
        z = torch.matmul(z, self.weights_W[self.num_layers_normal - 1]) + self.biases_W[self.num_layers_normal - 1]
        return z

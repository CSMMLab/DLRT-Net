...
from dlraNet import DLRANet, PartialDLRANet, ReferenceDLRANet
...
def main():
    ...
    model = ReferenceDLRANet(input_dim=input_dim, output_dim=output_dim, layer_width_dlra=layer_width,
    ...
    for t in range(epochs):
        print(f"Epoch {t + 1}\n-------------------------------")
        plain_train(train_dataloader, model, loss_fn, device)
        test(test_dataloader, model, loss_fn, device)
    print("Done!")

    # evaluate
    out_y = model.plain_forwad(tensor_test_x)
    plt.plot(test_x, test_y)
    plt.plot(test_x, out_y.detach().numpy())
    plt.show()
...
def plain_train(dataloader, model, loss_fn, device):

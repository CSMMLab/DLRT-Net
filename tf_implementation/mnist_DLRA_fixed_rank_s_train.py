...
from dlranet import DLRANetSTrain, create_csv_logger_cb
...
def test(start_rank, tolerance):
    ...
    dlra_layer_dim = 200
    model = DLRANetSTrain(input_dim=input_dim, output_dim=output_dim, low_rank=starting_rank,
...
def train(start_rank, tolerance, load_model):
    ...
    dlra_layer_dim = 784  # Neuronen count pro Layer
    model = DLRANetSTrain(input_dim=input_dim, output_dim=output_dim, low_rank=starting_rank,
                          dlra_layer_dim=dlra_layer_dim, tol=tol, rmax_total=max_rank)
    # Build optimizer
